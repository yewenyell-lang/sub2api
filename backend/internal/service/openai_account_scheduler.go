...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	var _debugSkipExcluded, _debugSkipNotSchedOpenAI, _debugSkipCodex, _debugSkipPrivacy, _debugSkipIncompat, _debugSkipTransport int
	for i := range accounts {
		...
		if !account.IsSchedulable() || !account.IsOpenAI() {
			_debugSkipNotSchedOpenAI++
			continue
		}
		if account.ShouldSkipOpenAIFreeSchedulingAtUsageThreshold() || s.service.isOpenAIAccountRuntimeBlocked(account) {
			_debugSkipCodex++
			continue
		}
	...
	slog.Debug("openai.scheduler_lb_filter_debug",
		"group_id", derefGroupID(req.GroupID),
		"model", req.RequestedModel,
		"total_accounts", len(accounts),
		"filtered", len(filtered),
		"skip_excluded", _debugSkipExcluded,
		"skip_not_sched_openai", _debugSkipNotSchedOpenAI,
		"skip_codex_threshold", _debugSkipCodex,
		"skip_privacy", _debugSkipPrivacy,
		"skip_incompat", _debugSkipIncompat,
		"skip_transport", _debugSkipTransport,
	)
	if len(filtered) == 0 {
		return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false)
	}
	...
	for _, candidate := range selectionOrder {
		fresh := s.service.resolveFreshSchedulableOpenAIAccountWithCodexCheck(ctx, candidate.account, req.RequestedModel, false, false)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
		fresh = s.service.recheckSelectedOpenAIAccountFromDBWithCodexCheck(ctx, fresh, req.RequestedModel, false, false)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
		if req.RequireCompact && openAICompactSupportTier(fresh) == 0 {
