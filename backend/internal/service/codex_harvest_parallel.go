...
func (s *OpenAIGatewayService) runParallelHarvest(ctx context.Context, req ManualHarvestRequest, account *Account, emit func(ManualHarvestProgress), collection harvestCollection) (err error) {
	...
	for _, model := range req.Models {
		...
		for out := range results {
			...
			r := out.result
			message, level, detail := describeCodexHarvestOutcome(r.Kind, "", r.Status, len(r.State), r.Shape.Blocks, openAICodexTicketTargetLength(account, cfg), openAICodexTicketExpectedBlocks(account), model, mihomo.NodeDisplayName(out.node))
			...
			recordCodexHarvestProbe(account, model, r.Kind, out.node, "", r.Status, len(r.State), r.Shape.Blocks, openAICodexTicketTargetLength(account, cfg), openAICodexTicketExpectedBlocks(account))
			if r.Kind == "success" {
