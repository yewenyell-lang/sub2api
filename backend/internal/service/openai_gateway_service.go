...
// openAIAccountSchedulingSkipReason centralises the schedulable / OpenAI / model /
// endpoint-capability / compact-support checks used during account selection.
func openAIAccountSchedulingSkipReason(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	...
	if paused, reason := shouldAutoPauseOpenAIAccountByQuota(ctx, account); paused {
		...
		return "openai_quota_auto_pause"
	}
	...
	if !account.SupportsOpenAIEndpointCapability(requiredCapability) {
		return "endpoint_capability_not_supported"
	}
...
func isOpenAIAccountEligibleForRequest(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	return openAIAccountSchedulingSkipReason(ctx, account, requestedModel, requireCompact, requiredCapability) == ""
}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if reason := openAIAccountSchedulingSkipReason(ctx, account, requestedModel, requireCompact, requiredCapability); reason != "" {
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability) (*Account, bool) {
	...
	var _debugExcluded, _debugFreshNil, _debugRecheckNil, _debugChannelRestrict, _debugCompactBlocked int
	...
	if selected == nil {
		slog.Debug("openai.select_best_account_debug",
			"group_id", derefGroupID(groupID),
			"model", requestedModel,
			"total", len(accounts),
			"excluded", _debugExcluded,
			"fresh_nil", _debugFreshNil,
			"recheck_nil", _debugRecheckNil,
			"channel_restrict", _debugChannelRestrict,
			"compact_blocked", _debugCompactBlocked,
		)
	}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky {
					skipReason := openAIAccountSchedulingSkipReason(ctx, account, requestedModel, requireCompact, requiredCapability)
	...
	for i := range accounts {
		...
		if acc.ShouldSkipOpenAIFreeSchedulingAtUsageThreshold() || s.isOpenAIAccountRuntimeBlocked(acc) {
			continue
		}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if !imageGenerationAllowed && !isOpenAIImageGenerationModel(reqModel) && stripOpenAIResponsesImageGenerationTools(reqBody) {
		bodyModified = true
		disablePatch()
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Stripped /responses image_generation tool without group image permission")
	}
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	apiKey := getAPIKeyFromContext(c)
	if !GroupAllowsImageGeneration(apiKeyGroup(apiKey)) && !isOpenAIImageGenerationModel(reqModel) {
		...
		if stripOpenAIResponsesImageGenerationTools(reqBody) {
			...
			logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Stripped passthrough /responses image_generation tool without group image permission")
		}
	}
