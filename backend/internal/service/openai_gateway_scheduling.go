...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if reason := openAICompatibleAccountEligibilityFailureReason(ctx, account, groupID, platform, requestedModel, false, requiredCapability); reason != "" {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky {
					stickyIneligibleReason = openAICompatibleAccountEligibilityFailureReason(ctx, account, groupID, platform, requestedModel, false, requiredCapability)
